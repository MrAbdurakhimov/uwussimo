...
export const getState =
  ...
  ({ id, name = '' }: { id?: string; name?: string }): SessionProcessState =>
    session.states[id || getProcessId(name)] || {};

export const saveState =
  (session: SessionState, updateSession: Dispatch<SessionAction>) =>
  ({ id, height = 0, width = 0, x = 0, y = 0 }: SessionProcessState): void => {
